...
def test_round_trip():
    ...
    cache.save_cached_inline_result(POST_URL, {"caption": "c", "title": "t", "items": ITEMS})
    cached = cache.get_cached_inline_result(POST_URL)

    assert cached == {"caption": "c", "title": "t", "items": ITEMS, "version": cache.INLINE_CACHE_VERSION}


def test_keyed_by_the_normalized_url():
    cache.save_cached_inline_result("https://instagr.am/p/ABC123", {"items": ITEMS})

    assert cache.get_cached_inline_result("https://www.instagram.com/someone/p/ABC123/?igsh=xyz") is not None


def test_saving_again_replaces_the_entry():
    cache.save_cached_inline_result(POST_URL, {"items": ITEMS})
    cache.save_cached_inline_result(POST_URL, {"items": [{"type": "video", "file_id": "v"}]})

    assert cache.get_cached_inline_result(POST_URL)["items"] == [{"type": "video", "file_id": "v"}]
    assert list(rows(cache.POST_CACHE.path)) == [POST_URL]


def test_an_entry_from_an_older_version_is_not_returned():
    cache.POST_CACHE.put(POST_URL, "3", {"items": ITEMS})

    assert cache.get_cached_inline_result(POST_URL) is None


def test_an_entry_without_items_is_not_returned():
    cache.POST_CACHE.put(POST_URL, cache.INLINE_CACHE_VERSION, {"items": []})
...
def test_entries_outlive_the_connection():
    ...
    cache.POST_CACHE.close()

    assert cache.get_cached_inline_result(POST_URL)["items"] == ITEMS
...
def test_an_entry_deleted_by_hand_is_gone_at_once():
    ...
    cache.save_cached_inline_result(POST_URL, {"items": ITEMS})
    assert cache.get_cached_inline_result(POST_URL) is not None

    by_hand(cache.POST_CACHE.path, "DELETE FROM posts WHERE url = ?", (POST_URL,))
...
def test_an_unusable_database_reads_as_a_miss_and_saving_does_not_raise(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cache, "POST_CACHE", cache.PostCache(tmp_path))
...
def test_a_row_mangled_by_hand_reads_as_a_miss():
    cache.save_cached_inline_result(POST_URL, {"items": ITEMS})
    by_hand(cache.POST_CACHE.path, "UPDATE posts SET result = 'not json'")
...
def legacy_cache(tmp_path):
    ...
    legacy = cache.PostCache(tmp_path / "carried.sqlite3", legacy_json=tmp_path / "inline_cache.json")
    yield legacy
    legacy.close()


def write_legacy(legacy, content):
    legacy.legacy_json.write_text(content if isinstance(content, str) else json.dumps(content), encoding="utf-8")
...
def test_the_json_cache_is_carried_over(legacy_cache):
    write_legacy(
        legacy_cache,
        {
            POST_URL: {"version": cache.INLINE_CACHE_VERSION, "caption": "c", "items": ITEMS},
            "https://www.instagram.com/p/OLD/": {"version": "3", "file_id": "old"},
        },
    )

    assert legacy_cache.get(POST_URL, cache.INLINE_CACHE_VERSION) == {"caption": "c", "items": ITEMS}
    # Carried over as it was: an old entry keeps its version and so still
    # misses, just as it did in the file.
    assert rows(legacy_cache.path) == {POST_URL: cache.INLINE_CACHE_VERSION, "https://www.instagram.com/p/OLD/": "3"}


def test_the_json_cache_is_read_only_when_the_database_is_created(legacy_cache):
    write_legacy(legacy_cache, {POST_URL: {"version": cache.INLINE_CACHE_VERSION, "items": ITEMS}})
    legacy_cache.get(POST_URL, cache.INLINE_CACHE_VERSION)
    by_hand(legacy_cache.path, "DELETE FROM posts")
    legacy_cache.close()

    assert legacy_cache.get(POST_URL, cache.INLINE_CACHE_VERSION) is None
...
def test_a_start_that_fails_mid_import_leaves_nothing_behind(legacy_cache, monkeypatch):
    ...
    with monkeypatch.context() as patched:
        patched.setattr(cache.PostCache, "_import_legacy_json", crash)
        with pytest.raises(RuntimeError):
            legacy_cache.get(POST_URL, cache.INLINE_CACHE_VERSION)

    # No table, so the next start creates it and imports the file again.
    assert legacy_cache.get(POST_URL, cache.INLINE_CACHE_VERSION) == {"items": ITEMS}
