...
Под каждым таким сообщением есть кнопка «Посмотреть карусель». Она открывает личный чат с ботом по deep-link (`t.me/<бот>?start=<тип>_<шорткод>`), и там приходит вся публикация целиком одним альбомом - из кэша `file_id`, без повторного скачивания. Кнопку может нажать и тот, кому сообщение отправлено: альбом придёт в его собственный чат с ботом. При первом переходе Telegram покажет кнопку «Запустить» - это обычное поведение для бота, с которым ещё не общались.

Отправить весь альбом прямо в тот чат, куда ушло inline-сообщение, нельзя: Telegram не сообщает боту, в какой чат оно было отправлено, а писать бот может только туда, где состоит сам.
